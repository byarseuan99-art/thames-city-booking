...
export function DataProvider({ children }: { children: React.ReactNode }) {
    ...
    const fetchData = async () => {
      try {
        const [usersRes, bookingsRes, waitlistRes, issuesRes, configsRes] = await Promise.all([
          ...
          fetch(`${API_URL}/api/bookings`).then(res => res.json()),
          fetch(`${API_URL}/api/waitlist`).then(res => res.json()),
          fetch(`${API_URL}/api/issues`).then(res => res.json()),
          fetch(`${API_URL}/api/approval-configs`).then(res => res.json())
        ]);
  ...
  const updateApprovalConfig = async (department: string, config: ApprovalConfig) => {
    try {
      const res = await fetch(`${API_URL}/api/approval-configs`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
